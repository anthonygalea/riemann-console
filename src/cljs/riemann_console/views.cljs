  ...
  (let [dashboard-settings? (re-frame/subscribe [::subs/dashboard-settings?])
    ...
    (when @dashboard-settings?
      [:div.fixed.top-0.bottom-0.right-0.left-0.bg-black-90.near-white.pa3.z-1
       [:div.mw6.bg-near-black.near-white.center.pa3.br1.ba.b--black
        ...
        [:form.ph0.w-100
         [:fieldset.ba.b--transparent.ph0.mh0
          [:div.mt3
           ...
           [:input.pa2.input-reset.ba.bg-dark-gray.w-100.near-white.b--black.br1
  ...
  (let [filter-value (reagent/atom nil)
    ...
    (fn [widget-id stream]
      [:div
       ...
       (let [fields (re-frame/subscribe [::subs/dashboard-widget-fields widget-id])]
...
(defn gauge [{:keys [id min max w h] :as widget} stream]
  [:div.w-100.h-100
...
(defn time-series [{:keys [id w h show-legend] :as widget} stream]
  [echart {:size [w h]
...
(defn widget [id]
  (let [w (re-frame/subscribe [::subs/widget id])
        stream (re-frame/subscribe [::subs/stream id])]
    [:div.overflow-y-scroll.bg-dark-gray
     {:style {:height "95%"}}
     (case (:type @w)
       :gauge [gauge @w @stream]
       :grid [grid @stream]
       :table [table id @stream]
       :time-series [time-series @w @stream])]))

(defn widget-panel [{:keys [id type title x y w h]}]
...
(defn widget-configurer-preview []
  (let [configuring-widget (re-frame/subscribe [::subs/configuring-widget])]
    [:div.pa2.w-100.ba.br1.b--black.bg-dark-gray.mt3
     {:style {:height "40%"}}
...
(defn widget-configurer []
  (let [configuring-widget (re-frame/subscribe [::subs/configuring-widget])
    ...
    (when @configuring-widget
      [:div.dib.fixed.top-0.bottom-0.bg-black-90.near-white.pa3.z-1.right-0.left-0
       [:div.bg-near-black.mw8.center.ma3.pa3.fixed.top-0.bottom-0.left-0.right-0.br1.ba.b--black
        [:h4.dib.near-white.ma0 "Configure widget"]
        [widget-configurer-close-button]
        [widget-configurer-preview]
        [:div.flex.flex-wrap.justify-between-l.justify-around.mt2
         [:div.w-50-l.w-100.pr2-l.pr0
